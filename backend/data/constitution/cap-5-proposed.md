...
**Block.** A container of data produced by a Stake Pool that includes, at minimum, a header.
...
**Epoch.** A Protocol-determined interval characterized by a fixed number of Slots.

**lovelace.** The smallest unit of value for the native cryptocurrency of the Cardano Blockchain.

**ada.** A superunit of lovelace, with 1 ada equal to 1,000,000 lovelace.

**Delegator.** A private key holder that delegates stake to a Stake Pool for block production and network security, to a DRep for participation in on-chain governance, or both.
...
**Guardrails.** A set of restrictions on Governance Actions to prevent undesirable outcomes and assist voters in deciding whether the proposed action complies with the Cardano Blockchain Ecosystem Constitution.

**Guardrails Script.** A smart contract script that checks specific proposed Governance Actions against automatically checkable Guardrails.

**Must/Must not.** Where this Appendix says that a value "must" or "must not" be set below or above some value, this means that the Guardrail is a requirement that will be enforced by Cardano Blockchain ledger rules or other built-in mechanisms where possible.

**Should/Should not.** Where this Appendix says that a value "should not" be set below or above some value, this means that the Guardrail is a recommendation or guideline.
...
The Guardrails Script only affects two types of governance actions: "Parameter Update" actions, and "Treasury Withdrawals" action.
...
- **(~)** The Guardrail Script cannot be used to enforce the Guardrail for the stated reason, but future ledger changes could enable this
...
PARAM-02a (y) Where a protocol parameter is explicitly listed in this document but no checkable Guardrails are specified, the Guardrails Script must not impose any constraints on changes to the parameter.
...
PARAM-03a (y) A parameter that is critical to the operation of the blockchain requires an SPO vote in addition to a DRep vote: SPOs must say "yes" with a collective support of more than 50% of all active block production stake.

PARAM-04a (x) At least 90 days should normally pass between the publication of an off-chain proposal to change a parameter that is critical to the operation of the blockchain and the submission of the corresponding on-chain governance action.
...
PARAM-05a (y) DReps must vote "yes" with a collective support of more than 50% of all active voting stake.

PARAM-06a (x) At least 90 days should normally pass between the publication of an off-chain proposal to change a parameter that is critical to the governance system and the submission of the corresponding on-chain governance action.
...
MPC-03 (x) *minPoolCost* should be set in line with the economic cost for operating a pool
...
TC-05 (~) *treasuryCut* must not be changed more than once in any 36 epoch period (approximately 6 months)
...
ME-04 (x) *monetaryExpansion* should not be varied by more than +/- 10% in any 73-epoch period (approximately 12 months)

ME-05 (x) *monetaryExpansion* should not be changed more than once in any 36-epoch period (approximately 6 months)
...
NETWORK-01 (x) No individual network parameter should change more than once per two epochs

NETWORK-02 (x) Only one network parameter should be changed per epoch unless they are directly correlated
...
MBBS-03a (x) *maxBlockBodySize* must not be decreased, other than in exceptional circumstances

MBBS-04 (~) *maxBlockBodySize* must be large enough to include at least one transaction

MBBS-05 (x) *maxBlockBodySize* should be changed by at most 10,240 bytes (10KB) per epoch
...
MTS-05 (x) *maxTxSize* should not be increased by more than 2,560 bytes (2.5KB) in any epoch

MTS-06 (x) *maxTxSize* should not exceed 1/4 of the block size
...
MEU-S-01 (~) *maxBlockExecutionUnits[steps]* must not be less than *maxTxExecutionUnits[steps]*
...
MBHS-03 (x) *maxBlockHeaderSize* must be large enough for the largest valid header

MBHS-04 (x) *maxBlockHeaderSize* should only normally be increased if the protocol changes
...
PPI-04 (x) *poolPledgeInfluence* should not vary by more than +/- 10% in any 18-epoch period (approximately 3 months)
...
PRME-02 (x) *poolRetireMaxEpoch* should not be lower than 1
...
MVS-03 (~) *maxValueSize* must be less than *maxTxSize*

MVS-04 (~) *maxValueSize* must not be reduced

#### Plutus Cost Models (costModels)

##### GUARDRAILS

PCM-01 (x) *Cost model* values must be set by benchmarking on a reference architecture

PCM-02 (x) The *cost model* must be updated if new primitives are introduced or a new Plutus language version is added

PCM-03a (~) *Cost model* values should not normally be negative

PCM-04 (~) A *cost model* must be supplied for each Plutus language version that the protocol supports
...
GD-04 (x) *govDeposit* should be adjusted in line with fiat changes
...
DRD-04 (x) *dRepDeposit* should be adjusted in line with fiat changes
...
DRA-04 (~) *dRepActivity* must be greater than *govActionLifetime*

DRA-05 (x) *dRepActivity* should be calculated in human terms (60 days, etc.)
...
GAL-03 (x) *govActionLifetime* should not be lower than 2 epochs (10 days)

GAL-04 (x) *govActionLifetime* should be calibrated in human terms (e.g., 30 days, two weeks)
...
CMTL-05a (x) *committeeMaxTermLength* should not exceed 220 epochs (approximately 3 years)
...
All other parameter changes should be monitored and the reversion plan implemented if the overall effect on performance, security, functionality, or long-term sustainability is unacceptable.

### 2.7. Non-Updatable Protocol Parameters

Some fundamental protocol parameters cannot be changed by the "Parameter Update" action. These parameters can only be changed in a new Genesis file as part of a hard fork.
...
HARDFORK-01 (~) The major protocol version must be the same as or one greater than the major version that will be enacted immediately prior to this change.

HARDFORK-02a (~) Unless the major protocol version is also changed, the minor protocol version must be greater than the minor version that will be enacted immediately prior to this change.
