...
export function renderCreate(state) {
 ...
 <div class="w-12 h-12 bg-white/15 rounded-2xl flex items-center justify-center shadow-lg text-on-surface ">
 <i data-lucide="${isCIS ? 'alert-octagon' : 'plus-square'}" class="w-6 h-6"></i>
                </div>
 <h1 class="text-3xl sm:text-5xl lg:text-6xl font-black italic tracking-tighter text-on-surface uppercase leading-none">
                    ${isCIS ? 'Flag Issue' : 'New Proposal'}
                </h1>
            </div>
 <p class="text-on-surface-variant text-xl font-medium">
                ${isCIS ? 'Identify a structural problem within the institutional layer.' : 'Draft a formal amendment for the Cardano Constitution.'}
            </p>
        </header>

        <!-- Type Toggle -->
 <div class="flex p-2 bg-white/80 rounded-[3rem] border border-slate-100 shadow-sm mb-12 max-w-md">
            <button onclick="window.setCreateType('CAP')"
 class="flex-1 py-4 rounded-[2.5rem] text-sm font-black uppercase tracking-widest transition-all ${!isCIS ? 'bg-blue-600 text-white shadow-xl' : 'text-slate-400 hover:text-slate-900 '}">
                CAP
            </button>
            <button onclick="window.setCreateType('CIS')"
 class="flex-1 py-4 rounded-[2.5rem] text-sm font-black uppercase tracking-widest transition-all ${isCIS ? 'bg-amber-500 text-white shadow-xl' : 'text-slate-400 hover:text-slate-900 '}">
                CIS
            </button>
        </div>
 ...
 class="w-full bg-slate-50 p-6 rounded-3xl min-h-[120px] font-medium text-lg outline-none border-2 border-transparent focus:border-blue-600 transition-all text-slate-900 resize-none">${escapeHtml(draft.abstract || '')}</textarea>
                </div>
            </div>
 ...
 <button type="button" onclick="window.setView('constitution')" class="text-[10px] font-black uppercase tracking-widest text-blue-600 hover:underline">
                ...
                ${refs.length === 0 ? `
 <div class="bg-white/50 border-2 border-dashed border-slate-200 p-16 rounded-[4rem] text-center">
 <p class="text-slate-400 font-bold text-sm italic">Highlight text in the Constitution to add a reference.</p>
                </div>
                ` : refs.map((ref, idx) => `
 <div class="bg-white/80 p-8 sm:p-12 rounded-[4rem] border border-slate-100 shadow-sm space-y-8 relative group">
 ...
 <div class="space-y-4">
 <span class="bg-slate-900 text-white text-[10px] font-black px-3 py-1 rounded-lg uppercase tracking-widest">Source: ${escapeHtml(ref.section)}</span>
 <div class="bg-slate-50 p-6 rounded-3xl italic text-slate-500 text-lg leading-relaxed border border-slate-100 ">"${escapeHtml(ref.text)}"</div>
                    </div>
 <div class="space-y-3">
 ...
 class="w-full bg-white/80 p-8 rounded-[2.5rem] min-h-[150px] font-medium text-lg outline-none border-2 border-blue-100 focus:border-blue-600 transition-all text-slate-900 shadow-inner">${escapeHtml(draft.revisions?.[ref.id] || '')}</textarea>
                    </div>
                </div>
                `).join('')}
            </div>
 ...
 class="w-full bg-slate-50 p-8 rounded-3xl min-h-[160px] font-medium text-lg outline-none border-2 border-transparent focus:border-blue-600 transition-all text-slate-900 ">${escapeHtml(draft.motivation || '')}</textarea>
                    </div>
 <div class="space-y-3">
 ...
 class="w-full bg-slate-50 p-8 rounded-3xl min-h-[160px] font-medium text-lg outline-none border-2 border-transparent focus:border-blue-600 transition-all text-slate-900 ">${escapeHtml(draft.analysis || '')}</textarea>
                    </div>
 <div class="space-y-3">
 ...
 class="w-full bg-slate-50 p-8 rounded-3xl min-h-[160px] font-medium text-lg outline-none border-2 border-transparent focus:border-blue-600 transition-all text-slate-900 ">${escapeHtml(draft.impact || '')}</textarea>
                    </div>
                </div>
                ` : `
 <div class="space-y-2">
 ...
 <span id="motivation-word-count" class="text-[10px] font-black ${(() => { const w = draft.motivation ? draft.motivation.trim().split(/\s+/).filter(Boolean).length : 0; return w > 500 ? 'text-red-500' : w > 400 ? 'text-amber-500' : 'text-slate-300 '; })()}">${draft.motivation ? draft.motivation.trim().split(/\s+/).filter(Boolean).length : 0} / 500 words</span>
                            </div>
                            <textarea name="motivation" id="create-motivation" required
                                oninput="window.updateDraftField('motivation', this.value); var w=this.value.trim()?this.value.trim().split(/\s+/).filter(Boolean).length:0; var el=document.getElementById('motivation-word-count'); if(el){el.textContent=w+' / 500 words'; el.className='text-[10px] font-black '+(w>500?'text-red-500':w>400?'text-amber-500':'text-slate-300 ');}"
                                placeholder="Explain why the constitution should be changed…"
 class="w-full bg-slate-50 p-8 rounded-3xl min-h-[180px] font-medium text-lg outline-none border-2 border-transparent focus:border-blue-600 transition-all text-slate-900 ">${escapeHtml(draft.motivation || '')}</textarea>
                        </div>
 <div class="space-y-3">
 ...
 class="w-full bg-slate-50 p-8 rounded-3xl min-h-[200px] font-medium text-lg outline-none border-2 border-transparent focus:border-blue-600 transition-all text-slate-900 ">${escapeHtml(draft.analysis || '')}</textarea>
                        </div>
                    </div>
                </div>
                `}
 ...
 class="w-full bg-slate-50 p-6 rounded-3xl min-h-[120px] font-medium text-lg outline-none border-2 border-transparent focus:border-blue-600 text-slate-900 resize-none">${escapeHtml(draft.exhibits || '')}</textarea>
                    </div>
                </div>
            </div>
 ...
 class="flex-1 group bg-white/80 text-slate-700 p-8 rounded-[3rem] text-xl font-black border border-slate-200 hover:bg-slate-50 transition-all flex items-center justify-center gap-4">
 <i data-lucide="eye" class="w-6 h-6 text-blue-600"></i> Preview
                </button>
                <button type="submit" ${state.loading?.submitting ? 'disabled' : ''}
 class="flex-[2] group bg-slate-950 text-white p-8 rounded-[3rem] text-2xl font-black shadow-2xl hover:-translate-y-1 active:scale-95 transition-all flex items-center justify-center gap-4 disabled:opacity-50">
                    ${state.loading?.submitting
 ? `<div class="w-6 h-6 border-4 border-slate-400 border-t-white rounded-full animate-spin"></div> Submitting…`
