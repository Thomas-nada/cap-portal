const SECTION_STYLE = {
    ...
    'using-the-portal': { icon: 'layout-dashboard', color: 'bg-slate-900 ' },
...
export function renderLearnHub(state) {
 ...
 <article id="guide-content" class="bg-white/80 p-10 rounded-[2rem] border border-slate-100 shadow-sm prose max-w-none">
                ${state.loading?.guide
 ...
 class="flex-1 p-4 rounded-2xl border-2 border-slate-200 text-slate-900 text-sm font-bold text-left hover:-translate-y-0.5 transition-all">
                    ${g.title}
                </button>
                ${canEdit ? `
                <button onclick="window.deleteGuide('${g.slug}')" title="Delete guide"
 class="opacity-0 group-hover/guide:opacity-100 w-8 h-8 rounded-xl bg-red-50 text-red-400 hover:text-red-600 hover:bg-red-100 flex items-center justify-center transition-all shrink-0">
 <i data-lucide="x" class="w-3 h-3"></i>
        ...
        if (isFaq) {
            return `
 <div class="bg-white/80 rounded-[3rem] border border-slate-100 shadow-sm p-10 md:col-span-2">
 ...
 <h2 class="text-2xl font-black italic tracking-tighter text-slate-900 uppercase mb-2">${sec.label}</h2>
 ...
 class="p-4 rounded-2xl border-2 border-dashed border-slate-200 text-slate-400 text-sm font-bold text-left hover:border-blue-300 hover:text-blue-500 transition-all">
                        + Add FAQ
                    </button>` : ''}
                </div>
            </div>`;
 ...
 <h2 class="text-2xl font-black italic tracking-tighter text-slate-900 uppercase mb-4">${sec.label}</h2>
 <div class="space-y-3">
                ${guideButtons}
                ${canEdit ? `
                <button onclick="window.openNewGuideModal()"
 class="w-full p-4 rounded-2xl border-2 border-dashed border-slate-200 text-slate-400 text-sm font-bold text-left hover:border-blue-300 hover:text-blue-500 transition-all">
 ...
 <h2 class="text-2xl font-black tracking-tighter text-slate-900 mb-2">No guides yet</h2>
 ...
 <h3 class="text-2xl font-black italic tracking-tighter text-slate-900 uppercase mb-6 text-center">Ready to Get Started?</h3>
